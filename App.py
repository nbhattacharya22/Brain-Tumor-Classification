...
if st.button("Predict", key="predict_button"): # Button to predict the image
    prediction = model.predict(image_array) 
    predicted_class = "No Tumor is Detected" if prediction[0] > 0.5 else "A Tumor Detected" #If the prediction is greater than 0.5, then the image is classified as having a tumor, otherwise it is classified as not having a tumor
    st.markdown(f"<h4 style='text-align: center;'>Based off the X-ray image uploaded: <b>{predicted_class}</b></h4>", unsafe_allow_html=True) # Display the prediction on the app
    
#show the graphs generated by the model
